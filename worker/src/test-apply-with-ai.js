...
async function main() {
  ...
  while (maxSteps-- > 0) {
    ...
    if (newHash === oldHash) {
      ...
      if (stuckCount >= 3) {
        console.log('  ⚠ Stuck in a loop. Looking for validation errors or checking what\'s wrong...');
        // Check for error messages, disabled buttons, etc.
        const errors = await page.evaluate(() => {
          const errs = [];
          document.querySelectorAll('[class*="error"], [class*="alert"], [role="alert"], .validation-error, span.text-danger, p.text-danger, div.text-danger').forEach(el => {
            if (el.textContent.trim()) errs.push(el.textContent.trim());
          });
          return errs;
        });
        if (errors.length > 0) {
          console.log(`  Found validation errors: ${errors.join(' | ')}`);
        }
        ...
        const btnDisabled = await page.evaluate(() => {
          ...
          return btn ? btn.disabled || btn.classList.contains('disabled') || btn.getAttribute('aria-disabled') === 'true' : 'button not found';
        });
        console.log(`  Continue button disabled? ${btnDisabled}`);

        console.log('\n============================================');
        console.log('  ⚠ STUCK — manual intervention needed');
        console.log('  Check the browser window and fill remaining fields');
        console.log('============================================');
        break;
      }
    } else {
