...
export default function AIEditPage() {
  ...
  const checkAccess = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Check if user has permission to edit resumes with AI
        if (!hasPermission(data.role, PERMISSIONS.EDIT_RESUME_WITH_AI)) {
  ...
  // Don't render if user doesn't have access (they'll be redirected)
  if (!userProfile || !hasPermission(userProfile.role, PERMISSIONS.EDIT_RESUME_WITH_AI)) {
    return null;
  }

  // Check if user can create new resume on edit
  const canCreateNew = hasPermission(userProfile.role, PERMISSIONS.CREATE_NEW_RESUME_ON_EDIT);
