...
- [ ] **Stripe webhook operates without verified secret** (`src/lib/stripe.js` + `.env.local`) — `STRIPE_WEBHOOK_SECRET` is commented out. Any POST to `/api/webhooks/stripe` can forge subscription upgrades.
