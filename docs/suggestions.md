...
- **2026-08-22 (found during logout investigation, not yet fixed):** `/api/auth/verify-token` response omits `role`, so after any proxy rotation `authResult.role` is undefined in proxy.js and ADMIN users get bounced from `/admin/*` to `/dashboard` on their first navigation after a 15-min idle (rotation path only). Include the role in `rotateRefreshToken()`'s return / the route response.
